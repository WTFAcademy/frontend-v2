    ...
    <section className="w-full flex flex-col bg-wtf-bg-code">
      ...
      <div className="grid grid-cols-2 md:flex md:flex-row">
        {Object.values(INFOS).map((info, index) => (
          <div
            key={index}
            className="flex-1 flex flex-col items-center gap-6 md:gap-8 p-8 md:px-10 md:py-14 border-wtf-border-divider border-[0.5px] border-solid"
          >
            {info.icon}
            <div className="text-wtf-text-1 text-lg font-bold leading-none text-center">{info.title}</div>
          </div>
        ))}
      </div>
    </section>
