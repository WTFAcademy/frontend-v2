  ...
  return (
    <section className="container flex">
      <div className="w-full flex flex-col space-y-8 md:space-y-0 md:flex-row mt-[-0.5px] border-wtf-border-divider border-[0.5px] border-solid">
        <div className="flex flex-col gap-y-2 w-full pb-8 pt-16 pl-8 md:pl-10 md:w-1/3 justify-center">
          <div className="text-4xl font-bold min-w-[170px]">{t.index.Sponsors}</div>
          <div className="text-base font-normal">
            {t.index.Sponsor_the_future_of_Web3_ecology}
          </div>
        </div>
        <div className="grid grid-cols-2 gap-0 w-full mt-0 md:w-2/3 lg:grid-cols-3 xl:grid-cols-4 !mb-[-0.5px]">
          {sponsors.map((sponsor) => (
            ...
            >
              <Image 
                ...
                width={120} 
                height={40} 
                className="object-contain w-auto h-min-[40px]" 
              />
            </Link>
          ))}
        </div>
